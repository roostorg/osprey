...
- Osprey no longer requires GCP to run: publishers are built through a `make_publisher` factory that falls back to a noop publisher (emitting a `configuration.errors` metric) when GCP credentials are missing, and can be turned off entirely with `OSPREY_PUBSUB_ENABLED=false` ([#388](https://github.com/roostorg/osprey/pull/388) by [@julietshen](https://github.com/julietshen))
