While Osprey worker can stand on its own by directly ingesting data from Kafka, Osprey Coordinator provides alternative that provides additional features such as load balancing and synchronous actions.
...
### Worker Connection
In `docker-compose.yaml`, set the worker's environment:
...
Workers connect to the coordinator automatically when configured to use `osprey_coordinator` as the input stream source.
