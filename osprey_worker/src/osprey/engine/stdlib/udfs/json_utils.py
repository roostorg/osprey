...
class FieldChainPath:
    """A `$.a.b.c` json path — single-field child accesses off the root — specialized into a tuple of keys.

    `get_first()` walks those keys with plain subscripts instead of running the jsonpath_rw
    interpreter, which does a `find()` call and allocates a `DatumInContext` per level. It
    duck-types the `JSONPath` surface (`find`/`update`/`str`/`==`) by delegating to the expression
    it was compiled from, so it stays a drop-in for anything holding a parsed path.
    """

    __slots__ = ('_expr', '_keys')

    def __init__(self, keys: Tuple[str, ...], expr: JSONPath):
        self._keys = keys
        self._expr = expr
    ...
    def find(self, data: Any) -> Any:
        return self._expr.find(data)

    def update(self, data: Any, val: Any) -> Any:
        return self._expr.update(data, val)
    ...
    def __eq__(self, other: object) -> bool:
        return bool(self._expr == (other._expr if isinstance(other, FieldChainPath) else other))


def _compile_field_chain(expr: JSONPath) -> Optional[Tuple[str, ...]]:
...
def parse_path(path: ConstExpr[str]) -> Union[JSONPath, FieldChainPath]:
    ...
    return expr if keys is None else FieldChainPath(keys, expr)
...
def get_from_data(
    expr: Union[JSONPath, FieldChainPath],
