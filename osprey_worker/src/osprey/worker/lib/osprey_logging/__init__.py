...
def get_route_metadata_logging_tags() -> list[tuple[str, str | None]]:
    ...
    if not has_request_context() or not hasattr(flask_global, DATADOG_ROUTE_METADATA_ATTR):
        return []
    return cast(list[tuple[str, str | None]], flask_global.get(DATADOG_ROUTE_METADATA_ATTR).items())


def configure_logging(
    package: types.ModuleType | str = 'osprey.worker.lib.osprey_logging',
