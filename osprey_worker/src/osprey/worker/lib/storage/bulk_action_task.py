...
class BulkActionJob(Model):
    ...
    error: Optional[str] = Column(Text, nullable=True)  # type: ignore[misc]
...
class BulkActionTask(Model):
    ...
    created_at: datetime = Column(DateTime(timezone=True), nullable=False)
    # sqlalchemy 1.4 mypy plugin doesn't read nullable= when inferring Mapped[T].
    completed_at: Optional[datetime] = Column(DateTime(timezone=True), nullable=True)  # type: ignore[misc]
