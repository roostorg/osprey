...
def get_user_id(entity_id: str, entity_type: str) -> int | None:
    return int(entity_id) if entity_type == EntityType.USER else None


def get_guild_id(entity_id: str, entity_type: str) -> int | None:
    return int(entity_id) if entity_type == EntityType.GUILD else None


def get_user_id_from_extracted_features(extracted_features: dict[str, Any]) -> int | None:
