...
from osprey.worker.adaptor.plugin_manager import bootstrap_label_provider
...
from osprey.worker.sinks.sink.base_sink import BaseSink
...
class BulkLabelSink(BaseSink):
    ...
    def __init__(
        ...
        self._label_output_sink = LabelEffectsOutputSink(self._label_provider)
        self._engine = engine
    ...
    def _process_task(self, task: BulkLabelTask) -> None:
        ...
        for current_entity_index in task.iterate_entity_indices():
            ...
            self._apply_label_mutations(entity_key, task)
    ...
    def _apply_label_mutations(self, entity_key: EntityKey, task: BulkLabelTask) -> None:
        def _log_before_sleep(retry_state: RetryCallState) -> None:
    ...
    def rollback_task_effects(
        ...
        label_output_sink = LabelEffectsOutputSink(label_provider)
        ...
        for rows_processed, row in enumerate(results):
            ...
            entity_key = EntityKey(type=entity_type, id=value)
            if entity_key.id in excluded_ids:
                rows_excluded += 1
                continue

            if include_ids is not None and str(entity_key.id) not in include_ids:
                rows_excluded += 1
                continue
