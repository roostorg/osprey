...
- Python version pinned in `.python-version`, managed with [`uv`](https://docs.astral.sh/uv/). CI verifies the lockfile, installs with `uv sync --dev --locked`, and runs `ruff` + `mypy` via `pre-commit` plus `fawltydeps`. Native asyncio and Docker/gevent pytest jobs opt into the non-default `coverage` dependency group and upload separate report-only branch-coverage XML artifacts; production images exclude that group and no coverage threshold is enforced.
- TypeScript in `osprey_ui/` (React + Ant Design + Highcharts; Node version in `.github/workflows/code-quality.yml`). CI runs `npm run format:check` and a production `npm run build`.
- Rust in `osprey_coordinator/` (stable toolchain, `tokio` + `tonic` + `etcd` + `rdkafka`). `cargo fmt`, `cargo build`, and `cargo test` gate CI; only `cargo clippy -- -D warnings` remains advisory because of its existing baseline.
