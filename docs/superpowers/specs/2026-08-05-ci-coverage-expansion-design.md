...
- the Docker/gevent suite measures the `osprey_worker/src/osprey/engine`, `osprey_worker/src/osprey/worker`, and `example_plugins/src` source trees and emits `/tmp/test-results/coverage-sync.xml` using the sync configuration.
