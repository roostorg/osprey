...
The worker consumes from a Kafka topic — `osprey.actions_input` by default (`OSPREY_KAFKA_INPUT_STREAM_TOPIC` in `docker-compose.yaml`; see `KafkaInputStream` in `osprey_worker/src/osprey/worker/sinks/sink/input_stream.py`). Each message is a JSON blob shaped like:
...
`action_name` is what your rules match against; the inner `data` object is whatever fields your event has — those become the values your UDFs and feature extractors read.
...
If you run Osprey with the optional Rust coordinator (`osprey_coordinator/`), external services can submit a single action directly and get an immediate response, without going through Kafka at all — useful when a caller needs a synchronous verdict rather than firing into a queue. This is the Sync Action API on port `19951` (bidirectional streaming for workers is a separate port, `19950`). See the [Coordinator README](../example_docker_compose/run_osprey_with_coordinator/README.md) for a working `grpcurl` example and full setup.

### Running against your own platform, without forking

If your events don't come from Kafka/PubSub/the coordinator, or don't arrive as that JSON envelope, you don't need to fork Osprey — there are two plugin hooks for exactly this:
...
You can register your own via the same hook — this is the extension point for pushing results into a review queue, a webhook, or any other external system.
