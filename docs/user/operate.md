...
> **Caution:** A false positive in a bulk action can label a large number of entities incorrectly. Always review the entity count before confirming a job.

### Starting a bulk action

There are two ways to start a bulk action:

1. **From the sidebar** — navigate to Bulk Actions and click the button to open the job creation modal
...
Before submitting, Osprey shows a count of how many unique entities will be labeled. Review this number carefully.
...
- **Left** — a list of recent bulk jobs with status, progress, and summary statistics; filterable by task ID
- **Right** — a detailed view of a single job, including its full results and logs
